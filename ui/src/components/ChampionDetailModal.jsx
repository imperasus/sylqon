          ...
          return (
            <div key={key} className="flex flex-col items-center gap-1" title={name}>
              <div className={`relative rounded-md ring-1 ${rank >= 0 ? RANK_STYLE[rank].split(" ")[0] : "ring-white/12"}`}>
                ...
                {rank >= 0 && (
                  <span className={`absolute -bottom-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-bg-2 text-3xs font-bold ring-1 ${RANK_STYLE[rank]}`}>
                    {rank + 1}
                  </span>
                )}
              </div>
              <span className="text-3xs font-bold tracking-widest text-white/40">{key}</span>
            </div>
          );
        })}
      </div>
...
function PowerCurve({ tags }) {
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-white/55">{curveSummary(c)}</p>
    </div>
  );
}
        ...
        {builds.slice(0, 3).map((b, i) => (
          <div key={`${b.pro_name}-${i}`} className="rounded-lg border border-white/8 bg-white/[0.015] px-3 py-2">
            <div className="flex items-center gap-2">
              ...
              {b.patch && <span className="ml-auto font-mono text-xs text-white/30">{b.patch}</span>}
            </div>
...
function ChampRow({ c, patch, right }) {
  return (
    <div className="flex items-center gap-2 rounded-md px-1.5 py-1 hover:bg-white/5">
      ...
      <span className="flex-1 truncate text-base text-white/75">{c.name}</span>
      {right}
    </div>
  );
}
    ...
    <div>
      ...
      <div className="mt-2 space-y-0.5">
        {rows.length
          ...
          : <div className="px-1 text-base text-white/30">{empty}</div>}
      </div>
    </div>
  );
}
...
function BuildPath({ build, patch }) {
  if (!build) return <div className="mt-2 px-1 text-base text-white/30">No cached build for this role yet.</div>;
...
export default function ChampionDetailModal({ champion, role, patch, onClose }) {
  ...
  const hardest = counters.filter((c) => (c.advantage ?? 0) < 0).slice(0, 4);
  const easiest = counters.filter((c) => (c.advantage ?? 0) > 0)
    .sort((a, b) => (b.advantage ?? 0) - (a.advantage ?? 0)).slice(0, 4);
  const duos = (details?.synergies || []).slice(0, 4);
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              {tier && <span className={`rounded border px-1.5 py-px text-sm font-bold ${tier.cls} ${tier.glow}`}>{tier.label}</span>}
              <span className="text-sm uppercase tracking-widest text-white/35">{ROLE_LABELS[role] || role}</span>
            </div>
            <div className="mt-1 flex items-center gap-3 text-base text-white/55">
        ...
        {!loading && details && !details.error && (
          <div className="grid min-h-0 gap-x-5 gap-y-4 overflow-hidden md:grid-cols-2">
            <AbilitiesBar slug={champion.slug} patch={patch} skillOrder={details.build?.skill_order || []} />

            {/* left: power curve + playstyle + build path */}
            <div className="flex min-w-0 flex-col gap-4">
              ...
              <MatchList title="BEST DUO PARTNERS" accent="ally" icon={Sparkles} rows={duos} patch={patch}
                         render={(c) => <span className="font-mono text-base text-good">{c.score}</span>} empty="No synergy data yet." />
            </div>

            {/* right: counter matchups + pro builds — split across columns so the
                modal fits without an inner scrollbar. */}
            <div className="flex min-w-0 flex-col gap-4">
