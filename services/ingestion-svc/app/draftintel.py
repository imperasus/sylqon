"""Web port of the sylqon draft-intel engine (pure, deterministic, offline).

This is a faithful reimplementation of ``sylqon/analysis/draft_intel.py`` plus
the team-summary aggregation from ``sylqon/lcu/lobby.py`` — the service must
stay independently containerizable (no ``sylqon`` import), so champion data
comes from the bundled ``app/data/draft_tables.json`` generated by
``python -m sylqon.tools.export_draft_tables`` (regenerate on patch bumps and
on any engine/threat-table change).

Parity contract: ``tests/fixtures/draft_parity.json`` records the sylqon-side
engine's outputs for a spread of drafts; ``tests/test_draftintel.py`` asserts
this port reproduces them exactly, and the sylqon suite asserts the fixture
still matches the source engine. Behaviour changes belong in the sylqon engine
first, then a regen — never here.

Picks are plain dicts ``{"name", "tags", "threats", "damage_type"}``; use
:func:`profile_by_name` / :func:`profile_by_id` to synthesise them from the
bundle. All numbers here score *compositions*, never players; ``win_pct`` is
deliberately clamped to [35, 65] — a draft heuristic, not a real model.
"""
...
_DATA = Path(__file__).resolve().parent / "data" / "draft_tables.json"
...
def _name_index() -> dict:
    """Both the display name ("Dr. Mundo") and the DDragon slug ("DrMundo" —
    what Match-V5 ``championName`` and thus ``match_participants`` store) map
    to the same profile; display names win a collision."""
    index: dict = {}
    for prof in _bundle()["champions"].values():
        slug = (prof.get("slug") or "").lower()
        if slug:
            index.setdefault(slug, prof)
    for prof in _bundle()["champions"].values():
...
def patch() -> str:
    """Data Dragon patch the bundle was generated from (e.g. '16.13.1')."""
    return _bundle().get("patch", "")


def _as_pick(prof: dict) -> dict:
    return {"name": prof["name"], "tags": list(prof["tags"]),
            "threats": list(prof["threats"]), "damage_type": prof["damage_type"]}


def profile_by_name(name: str | None) -> dict | None:
    """Synthesised pick dict for a display name (case-insensitive), or None."""
    if not name:
        return None
    prof = _name_index().get(name.lower())
    return _as_pick(prof) if prof else None


def profile_by_id(champion_id: int | str | None) -> dict | None:
    """Synthesised pick dict for a numeric champion id, or None."""
    if champion_id is None:
        return None
    prof = _bundle()["champions"].get(str(champion_id))
    return _as_pick(prof) if prof else None


def identity(name: str | None) -> dict | None:
    """Canonical ``{"name": display, "slug": ddragon-id}`` for any accepted
    spelling (display name or Match-V5 slug), or None if unknown."""
    if not name:
        return None
    prof = _name_index().get(name.lower())
...
def summarize_team(picks: list) -> dict:
    ...
    ad = sum(1 for p in picks if p["damage_type"] == "AD")
    ap = sum(1 for p in picks if p["damage_type"] == "AP")
    return {
        "physical_threats": ad,
        "magic_threats": ap,
        "mixed_threats": len(picks) - ad - ap,
        "heavy_cc_count": sum(1 for p in picks if "heavy_cc" in p["threats"]),
        "suppression": any("suppression" in p["threats"] for p in picks),
        "burst_ad": any("burst_ad" in p["threats"] for p in picks),
        "burst_ap": any("burst_ap" in p["threats"] for p in picks),
        "heavy_healing": any("heavy_healing" in p["threats"] for p in picks),
        "tanks": sum(1 for p in picks if "tank" in p["threats"]),
        "frontline": sum(1 for p in picks if set(p["tags"]) & {"Tank", "Fighter"}),
    }
...
def classify_comp(picks: list) -> dict:
    ...
    heavy_poke = set(_bundle()["heavy_poke"])
    split_push = set(_bundle()["split_push"])
    tags = [set(p["tags"]) for p in picks]
    threats = [set(p["threats"]) for p in picks]
    names = [p["name"] for p in picks]

    poke = sum(1 for i, n in enumerate(names)
               if n in heavy_poke or "poke" in threats[i])
    ...
    splitters = sum(1 for n in names if n in split_push)
...
# The signed edge is mapped to a win probability by a calibratable logistic (the
# correct functional form for a bounded probability). Kept in exact numeric parity
# with ``sylqon.analysis.win_model`` — this port cannot import sylqon, so the
# model is inlined here; both are re-derived from the same coefficients.
_SIGMOID_K = 0.24
_WIN_PCT_FLOOR, _WIN_PCT_CEIL = 20.0, 80.0


def _edge_to_win_pct(edge: float) -> int:
    pct = (1.0 / (1.0 + math.exp(-_SIGMOID_K * edge))) * 100.0
    return int(round(max(_WIN_PCT_FLOOR, min(_WIN_PCT_CEIL, pct))))
...
def draft_balance(ally_comp: dict, enemy_comp: dict,
    ...
    (a lane edge, roughly [-10, 10]) is folded in when available. Pure and
    network-free, mirroring :func:`classify_comp`.
